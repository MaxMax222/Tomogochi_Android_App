...
namespace FinalProj_Tomogochi.Classes
{
	public class BGupdateFBlistener : Java.Lang.Object, IEventListener
	{
		...
		{
            FirebaseHelper.GetFirestore().Collection($"users/{FirebaseHelper.GetFirebaseAuthentication().CurrentUser.Uid}/characters/{User.GetUserInstance().ActiveCharacter.Name}/lastBGs").OrderBy("createdAt").AddSnapshotListener(this);
        }
